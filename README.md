...
- Generic exception type can be used:
  - ❌ Do not throw System.Exception, System.SystemException, ApplicationException, NullReferenceException types.
  - ❎ Use InvalidOperationException if the object is in an inappropriate state.
...
- Exception that are thrown then catched by our code should be a dedicated exception type in order to allow developer to implement dedicated error management code in catch block. Example:
...
- If a public method expose an interface, you should put the documentation on the interface and add 
  ````///<inheritdoc>```` to the public method implementation.
...
- [Premature optimization is the root of all evil](https://www.google.com/search?q=premature+optimization+is+the+root+of+all+evil) 
